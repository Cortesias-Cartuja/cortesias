...
function cargarCoches() {
    db.ref('coches').on('value', snap => {
        ...
        snap.forEach(child => {
            ...
            c.id = child.key;
            cacheCoches.push(c);
            // Si estamos en salida, mostrar disponibles. Si estamos en entrada, mostrar prestados.
...
function seleccionarCoche() {
    ...
    if(modoActual === 'salida') {
        ...
        // Al entrar, buscamos el contrato abierto para este coche
        db.ref('contratos').orderByChild('matricula').equalTo(coche.matricula).once('value', snap => {
            snap.forEach(child => {
                const con = child.val();
                if(!con.fechaEntrada) { // Contrato sin cerrar
...
function finalizarEntrada() {
    const idC = document.getElementById('sel-vehiculos').value;
    if(!contratoActivoID) return alert("No se encontró el contrato de salida");
